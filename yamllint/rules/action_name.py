...
def check(conf, token, prev, next, nextnext, context):
    if isinstance(token, yaml.DocumentStartToken):
        context['workflow-name'] = False

    if isinstance(prev, yaml.DocumentStartToken):
        if isinstance(token, yaml.BlockMappingStartToken):
            if isinstance(next, yaml.KeyToken):
                if isinstance(nextnext, yaml.ScalarToken) and nextnext.value == 'name':
                    context['workflow-name'] = True

    if isinstance(token, (yaml.StreamEndToken, yaml.DocumentEndToken)):
        if not context['workflow-name'] == True:
            yield LintProblem(token.start_mark.line + 1,
                              token.end_mark.column + 1,
                              'workflow missing name key')

    if isinstance(next, yaml.ScalarToken) and next.value == 'name':
        print("-----")
        print(prev)
        print(token)
        print(next)
